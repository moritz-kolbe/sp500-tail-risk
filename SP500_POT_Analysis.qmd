...
This project demonstrates how risk measure inference, based on POT-modelling, 
is influenced by accounting for the dependence structure of time-series index data. 
To illustrate this, two approaches are compared: the first whitens the data prior 
to POT-analysis, i.e. it is preceded by a simple GARCH(1,1)-model. The second 
approach does not account for this volatility-dependence structure. Based on the 
results of the POT-model, $\text{VaR}_{\alpha}$ and $\text{ES}_{\alpha}$ are 
calculated as risk measures in both cases. The measures are validated via backtesting 
and evaluated using the Kupiec- and Christoffersen-Test. Finally, for both approaches, 
a 10-day forecast is produced.
...
from VaR_backtest_utils_correct import backtest_single, init_worker # git-repository
```
...
```{python}
prices = yf.download("^GSPC", start="1980-01-01")["Close"]  # on single stocks Adj.Close
...
## Volatility-filtered POT-Analysis
### Pre-Whitening
