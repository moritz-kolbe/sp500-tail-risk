...
print(f"Next-day VaR_{alpha}: {np.median(VaR_posterior):.2f}% "
      f"[{np.percentile(VaR_posterior, 2.5):.2f}%, "
      f"{np.percentile(VaR_posterior, 97.5):.2f}%]")

print(f"Next-day ES_{alpha}: {np.median(ES_posterior):.2f}% "
...
var_estimates, violations = zip(*results)
var_estimates = np.array(var_estimates)
violations = np.array(violations)

valid = ~np.isnan(var_estimates)
violations_clean = violations[valid]
var_clean = var_estimates[valid]
```
...
dates_test = returns_dec.index[window:][valid]
actual_losses = -returns_dec.values[window:][valid]
